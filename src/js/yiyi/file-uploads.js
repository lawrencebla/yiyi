(function(global, $) {
	...
	function appendChild(container) {
		return function(item) {
			...
			template.prop('id', 'file_upload_item' + item.id);
			template.find('.control-block-button').attr('data-id', item.id);
	...
	global.yjax('file_list', function(data) {
		if( data.code === 0 ) {
